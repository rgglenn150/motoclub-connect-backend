...
describe('Comprehensive Integration Tests', () => {
  ...
  describe('User Model Compatibility Tests', () => {
    ...
    it('should prevent duplicate Facebook accounts', async () => {
      ...
      try {
        ...
        throw new Error(`Duplicate Facebook account test failed: ${error.message}`);
      }
    });
  });
  ...
  describe('API Integration Tests', () => {
    ...
    before(async () => {
      ...
      const signupRes = await request(app)
        ...
        .send({ email, password, username, firstName: 'Test', lastName: 'User' });
    ...
    it('should create club with authenticated user', async () => {
      ...
      if (![200, 201].includes(res.status)) {
        throw new Error(`Club creation failed: ${res.status} ${JSON.stringify(res.body)}`);
      }
    ...
    it('should list clubs for authenticated user', async () => {
      ...
      if (res.status !== 200) {
        throw new Error(`Club listing failed: ${res.status} ${JSON.stringify(res.body)}`);
      }
    });
    ...
    it('should list users for authenticated user', async () => {
      ...
      if (res.status !== 200) {
        throw new Error(`User listing failed: ${res.status} ${JSON.stringify(res.body)}`);
      }
  ...
  describe('Error Handling Tests', () => {
    ...
    it('should handle malformed authorization headers', async () => {
      ...
      if (res.status !== 401) {
        throw new Error(`Expected 401 for malformed auth header, got ${res.status}`);
      }
    });
    ...
    it('should handle missing required fields in signup', async () => {
      ...
      if (res.status !== 400) {
        throw new Error(`Expected 400 for incomplete signup, got ${res.status}`);
      }
    });
