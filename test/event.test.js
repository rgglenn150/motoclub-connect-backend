...
describe('Event routes', () => {
  ...
  it('POST /api/event/create should create an event', async () => {
    ...
    const clubRes = await request(app)
      ...
      .send({ name: `Club ${Date.now()}`, description: 'Test', location: 'NY', isPrivate: true });
    const clubId = clubRes.body._id || clubRes.body.id;

    const res = await request(app)
      .post('/api/event/create')
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Ride', description: 'Fun', clubId, startTime: new Date(), endTime: new Date(), location: 'NY' });
