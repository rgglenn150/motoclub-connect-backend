...
describe('Auth routes', () => {
  ...
  it('POST /api/auth/login should return token for valid credentials', async function () {
    ...
    while (retries > 0) {
      try {
        // Ensure connection before attempting signup
        await ensureConnection();
        ...
        if (signupRes.body.message && signupRes.body.message.includes('connection')) {
        ...
        if (error.message.includes('connection') && retries > 1) {
  ...
  it('POST /api/auth/facebook/register should handle missing access token', async () => {
    const res = await request(app).post('/api/auth/facebook/register').send({
      username: 'testuser',
    });
    if (res.status !== 400) {
      console.error(
        'Facebook register without token response:',
        res.status,
        res.body
      );
    }
  ...
  it('POST /api/auth/facebook should handle missing access token', async () => {
    ...
    if (res.status !== 400) {
      console.error(
        'Facebook login without token response:',
        res.status,
        res.body
      );
    }
  ...
  it('POST /api/auth/facebook should handle invalid access token', async () => {
    ...
    if (![400, 404].includes(res.status)) {
      console.error(
        'Facebook login with invalid token response:',
        res.status,
        res.body
      );
    }
  ...
  it('POST /api/auth/facebook/register should handle invalid access token', async () => {
    ...
    if (res.status !== 400) {
      console.error(
        'Facebook register with invalid token response:',
        res.status,
        res.body
      );
    }
